...
const Register = ({ nextStep, setStep }) => {
    ...
    useEffect(() => {
        if (loading) {
            ...
            if (response === 'success') {
                dispatch(createToast('Usuario registrado con exito'));
                dispatch(loginUserThunk({email: body.email, password: body.password}))
                setLoading(false);
                setStep(3)
            }
            else {
    ...
    return (
        <section className='flex flex-col items-center justify-start min-h-screen w-screen overflow-y-scroll py-4'>
            ...
            <form onSubmit={handleSubmit} className='flex flex-col min-w-[250px]:full max-w-[460px]  justify-start items-center bg-white text-black rounded-lg shadow-lg z-50'>
                ...
                <article className='w-full px-10 py-10  flex flex-col justify-start items-center text-base text-roboto gap-1 bg-none'>
                    ...
                    <p className='text-roboto text-[#969696] text-sm text-center'>Ya tienes una cuenta? <span onClick={nextStep} className='text-[#000000] font-bold font-roboto cursor-pointer'>Inicia Sesion</span></p>
                    <hr className='w-full border-[#E0E0E0] my-2' />
                    <button
